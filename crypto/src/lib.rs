...
// impl BlsSign {
//     pub fn new(digest: &Digest, secret: &BlsPriKey, pubkey: &BlsPubKeyG1) -> (Self, ZkProofDLEq) {
//         let sks = SecretKeyShare::from_bytes(secret.0).unwrap();
//         let sig = sks.sign_g1(&digest);
//         // let Sign = BlsSign(sig);

//         let pk = pubkey.to_pubkey();
...
//     pub fn verify(&self, digest: &Digest, public_key: &PublicKeyShareG2) -> Result<(), BlsError> {
...
//         &self,
//         digest: &Digest,
//         public_key: &BlsPubKeyG1,
//         nizk: ZkProofDLEqWrapper,
//     ) -> Result<(), BlsError> {
//         let sig = SignatureShareG1::from_bytes(self.0).unwrap();
//         let key = PublicKeyShareG1::from_bytes(public_key.0).unwrap();
//         let g = G1Affine::generator();
//         let g_x = key.0 .0;
//         let h_x = sig.0 .0;
//         let nizk = nizk.into();
...
// pub fn calc_nizk(pubkey: BlsPubKeyG1, sign: BlsSign, digest: Digest, sk: BlsPriKey) -> ZkProofDLEq {
//     let pk = pubkey.to_pubkey();
//     let sig = sign.as_signature();
//     let sks = SecretKeyShare::from_bytes(sk.0).unwrap();
//     let mut rng = blsttc_thread_rng();
//     let r: Fr = Fr::random(&mut rng);
//     let g = G1Affine::generator();
//     let h = hash_g1(digest);
